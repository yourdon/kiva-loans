...
colnames(langs) ="lang"
ggplot(data=langs, aes(x=lang)) + geom_histogram(fill="lightblue") + labs(x="language")
...
ggplot(data=paid, aes(x=b.female)) + geom_bar(fill="lightblue") +
  labs(x="Ratio of female borrowers")
...
ggplot(data=newpaid, aes(x=paid_timediff)) + geom_bar(fill="lightblue") +
  labs(x="Repayment time")
summary(newpaid$paid_timediff)
```

Looking at the IQR, we see that most loans are repaid in the 6-12 month range, though some take 2 years or more to be repaid (and there are a few pesky negative values). We could consider segmenting by repayment time. 
...
As a last step, we'll write our most polished dataframe to a new JSON file for use in the next round of analysis.
