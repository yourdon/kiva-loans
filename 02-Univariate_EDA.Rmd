...
sum(is.na(loans[loans$status == "paid", ])) / (14197*18)
```
...
mean(partners); sd(partners); min(partners)
```

We see a lot of different partners with a large variance in their loan activity. It's conceivable that more active partners have different behaviors than less active partners do. This could be something to investigate.
...
countries_df = as.data.frame(table(as.factor(loans$location.country_code)))
...
There is a long tail of loans that take 10 days or longer to get funded (though none takes longer than 44 days). What might be going on with these?
...
Looking at the IQR, we see that most loans are repaid in the 7-10 month range, though some take 2 or even 3+ years to be repaid. We could consider segmenting by repayment time.
...
After a high-level survey of each variable, we have found a bunch of NAs but no data that demand to be excluded. The exception is our `disbursal_timediff` variable. The negative values here suggest that we cut our supply chain into two disconnected components; the first component, characterized by the `funded_timediff` variable, describes how a loan goes from being posted to being funded (even if it has already been disbursed), while the second component, characterized by the `paid_timediff` variable, describes how a loan goan from being disbursed to being repaid (regardless of when it was posted and funded).
