...
Smooth sailing. We'll now loop through all the files in the directory, run the preprocessing function on them, and concatenate them into one dataframe.
...
Now we have a 152,000 x 18 dataframe with all loans and all preliminary variables of interest. The fact that no exceptions were thrown suggests that all the other JSON files were similarly formatted. One thing that we should confirm, though, is that we have no duplicate records.
