...
Let's revisit our scatterplot matrix, only now we will add "pipeline features" -- that is, the `timediff`s that represent the difference in days between the different stages in the loan pipeline. We can also add in the number of lenders.
...
```{r, warning=FALSE, error=FALSE}
head(hc_loans)
hc_loans2 = hc_loans[hc_loans$funded_timediff >= 0, ]
